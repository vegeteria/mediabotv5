...
class Downloader:
    ...
    async def download_movie(
        ...
        **run_kwargs,
        ...
        downloadable_details_inst = DownloadableVideoFilesDetail(self.client_session)
        ...
        if download_caption or caption_only:
            caption_details_inst = DownloadableCaptionFileDetails(self.client_session)
            caption_details = await caption_details_inst.get_content_model(
                target_subject_id, resource=target_media_file
            )
            for lang in language:
                try:
                    target_caption_file = caption_details.get_subtitle_by_language(lang)
                    if not target_caption_file:
                        raise ZeroCaptionFileError(f"Caption file for {lang} is missing")
    ...
    async def download_tv_series(
        ...
        **run_kwargs,
        ...
        downloadable_files_detail_inst = DownloadableVideoFilesDetail(
        ...
        for req_params in download_request_params.request_params:
            ...
            for media_file in files_detail.list[req_params.offset :][
                ...
                if download_caption or caption_only:
                    caption_details_inst = DownloadableCaptionFileDetails(self.client_session)
                    caption_details = await caption_details_inst.get_content_model(
                        target_dub.subject_id, resource=media_file
                    )
                    ...
                    for lang in language:
                        ...
                        current_episode_details.setdefault("captions", []).append(subtitle_details)
                        subtitle_details_items.append(subtitle_details)
